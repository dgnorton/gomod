...
func Test_LoadGoSum(t *testing.T) {
	t.Parallel()

	tmpdir := t.TempDir()
	goSumFile := tmpGoSumFile(tmpdir, t)

	gosum, err := LoadGoSum(goSumFile)
	if err != nil {
		t.Fatal(err)
	}

	for k, _ := range gosum.Modules {
		prefix, pathMajor, ok := module.SplitPathVersion(k.Path)
		if ok {
			fmt.Printf("pre = %s, maj = %s\n", prefix, pathMajor)
		} else {
			println("not okay ****************")
		}
	}
...
func tmpGoSumFile(tmpdir string, t *testing.T) string {
	t.Helper()
	...
	tmpGoSum, err := ioutil.TempFile(tmpdir, "*_go.sum")
	if err != nil {
		t.Fatal(err)
	}
	defer tmpGoSum.Close()

	_, err = tmpGoSum.WriteString(`cloud.google.com/go v0.26.0/go.mod h1:aQUYkXzVsufM+DwF1aE+0xfcU+56JwCaLick0ClmMTw=
...
github.com/prometheus/prometheus v0.0.0-20200609090129-a6600f564e3c h1:x3RZiripR6DQd/8nYVNzdOla4HfBHBKcfE9cO6w7dSI=
